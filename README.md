...
The Anchor extension uses the Gemini API to generate subtasks and detect distractions. For this to work, you need to provide your personal Gemini API Key. This key is stored locally in your browser and is not sent anywhere else.
...
2.  **Enter the API Key in Settings:**
    - Click the Anchor extension icon in your Chrome toolbar.
    - Click the **⚙️ settings** link in the top-right corner of the popup.
    - Paste your API key into the **API Key** field.
    - Configure your preferred Pomodoro work/break durations if desired.
    - Click **Save Settings**.

3.  **Reload the Extension (recommended):**
    - Go to `chrome://extensions` and click the **Reload** button for Anchor.

> **Alternative (advanced):** You can also set the key via the Service Worker console:
>
> ```javascript
...
5.  **Break Phase Ends:** After the break, you'll receive another notification, and the timer will automatically restart a new work phase for the _same task_. **Distraction detection will turn back ON.**
...
- **"API key not found in storage." or API calls failing:**
  - Open **Settings** from the extension popup and confirm your API key is saved.
  ...
  - Reload the extension at `chrome://extensions` after saving your key.
- **"NaN:NaN" on Timer:**
...
This project uses ESLint and Prettier for code quality. You only need Node.js installed.
...
**Tip:** Install the ESLint and Prettier extensions in VS Code/Cursor so files format on save.
